...
void GazeboRosRealsense::OnNewFrame(const rendering::CameraPtr cam,
  ...
  if(camera_id==COLOR_CAMERA_NAME) this->image_color=image_msg_;
                                              ...
                                              uint32_t step_arg, void *data_arg)
{
  ...
  for (uint32_t j = 0; j < rows_arg; j++)
  {
    ...
    for (uint32_t i = 0; i < cols_arg; i++, ++iter_x, ++iter_y, ++iter_z, ++iter_rgb)
    {
      ...
      uint8_t *image_src = (uint8_t *)(&(this->image_color.data[0]));
      if (this->image_color.data.size() == rows_arg * cols_arg * 3)
      {
        // color
      ...
      else if (this->image_color.data.size() == rows_arg * cols_arg)
      {
