...
void GeoGraspEvo::compute() {
  ...
  while(name != true){
    cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_tillThumbCandidates"+std::to_string(objNumber)+".pcd";
    //cad2 = std::string("b")+"_objXYZ"+std::to_string(objNumber)+".pcd";
    bool exist = boost::filesystem::exists(cad);
    if (exist == 0) {
      name = true;
    } 
    else {
  ...
  /*pcl::io::savePCDFileASCII (cad, *this->objectCloud);
  std::cerr << "Saved " << (*this->objectCloud).size () << " data points to " <<cad2<<"." << std::endl;*/
  /*******************************************************************************************************************/
  ...
  /*******************************************************************************************************************/  
  // Nube voxelizada (a color - aunque en realidad deberia ser sin campo RGB) con centroide y eje mayor
  //pcl::PointCloud<pcl::PointXYZRGB>::Ptr colouredPC(new pcl::PointCloud<pcl::PointXYZRGB>);
  cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_obj"+std::to_string(objNumber)+".pcd";
  ...
  voxelizeCloud<pcl::PointCloud<pcl::PointXYZRGB>::Ptr, 
                pcl::VoxelGrid<pcl::PointXYZRGB> >(colouredPC, this->voxelRadius, voxelCloud2);
  ...
  *voxelCloud2 = *newScene;      
  ...
  cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_objVox"+std::to_string(objNumber)+".pcd";
  cad2 = std::string("b")+"_objVox"+std::to_string(objNumber)+".pcd";
  
  pcl::io::savePCDFileASCII (cad, *voxelCloud2);
  std::cerr << "Saved " << (*voxelCloud2).size () << " data points to " <<cad2<<"." << std::endl;
                
  float radius = 0.005;
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr centroid(new pcl::PointCloud<pcl::PointXYZRGB>);
  for(float theta=0; theta<2*3.14; theta=theta+2.5*0.017){
    for(float phi=0; phi<3.14; phi = phi+2.5*0.017){
    
      pcl::PointXYZRGB punto;
      punto.x = radius * cos(theta) * sin(phi) + (this->objectCentroidPoint).x;
      punto.y = radius * sin(theta) * sin(phi) + (this->objectCentroidPoint).y;
      punto.z = radius * cos(phi) + (this->objectCentroidPoint).z;
      punto.r = 255;
      punto.g = 255;
      punto.b = 0;
      
      (centroid->points).push_back(punto);
    }
  }
  (*voxelCloud2) += (*centroid);
  
  /*cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_centroid"+std::to_string(objNumber)+".pcd";
  cad2 = std::string("b")+"_centroid"+std::to_string(objNumber)+".pcd";
  
  pcl::io::savePCDFileASCII (cad, *voxelCloud2);
  std::cerr << "Saved " << (*voxelCloud2).size () << " data points to " <<cad2<<"." << std::endl;*/
  
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr majorAxis(new pcl::PointCloud<pcl::PointXYZRGB>);        
  for(float length=0; length<0.15; length=length+0.001){
    float radius = 0.0005;
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene(new pcl::PointCloud<pcl::PointXYZRGB>);
    for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
      for(float phi=0; phi<3.14; phi = phi+5*0.017){
        ...
        pcl::PointXYZRGB punto;
        punto.x = radius * cos(theta) * sin(phi) + (this->objectAxisCoeff)->values[0] + ((this->objectAxisCoeff)->values[3])*length;
        punto.y = radius * sin(theta) * sin(phi) + (this->objectAxisCoeff)->values[1] + ((this->objectAxisCoeff)->values[4])*length;
        punto.z = radius * cos(phi) + (this->objectAxisCoeff)->values[2] + ((this->objectAxisCoeff)->values[5])*length;
        punto.r = 255;
        punto.g = 0;
        punto.b = 255;
  ...
  *voxelCloud2 += *majorAxis; 
  
  /*pcl::PointCloud<pcl::PointXYZRGB>::Ptr majorAxis(new pcl::PointCloud<pcl::PointXYZRGB>);
  for(float length=0; length<0.2; length=length+0.001){
    
      pcl::PointXYZRGB punto;
      punto.x = (this->objectAxisCoeff)->values[0] + ((this->objectAxisCoeff)->values[3])*length;
      punto.y = (this->objectAxisCoeff)->values[1] + ((this->objectAxisCoeff)->values[4])*length;
      punto.z = (this->objectAxisCoeff)->values[2] + ((this->objectAxisCoeff)->values[5])*length;
      punto.r = 255;
      punto.g = 0;
      punto.b = 0;
      
      (majorAxis->points).push_back(punto);
  }
  (*voxelCloud2) += (*majorAxis);*/
  
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr majorAxis1(new pcl::PointCloud<pcl::PointXYZRGB>);        
  for(float length=0; length<0.15; length=length+0.001){
    float radius = 0.0005;
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene(new pcl::PointCloud<pcl::PointXYZRGB>);
    for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
  ...
  *voxelCloud2 += *majorAxis1; 
  ...
  for(float length=0; length<-0.2; length=length-0.001){
      ...
      (majorAxis1->points).push_back(punto);
  }
  (*voxelCloud2) += (*majorAxis1);*/
  
  cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_centroidPlusAxis"+std::to_string(objNumber)+".pcd";
  cad2 = std::string("b")+"_centroidPlusAxis"+std::to_string(objNumber)+".pcd";
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr colouredPC3(new pcl::PointCloud<pcl::PointXYZRGB>);
  cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_objVox"+std::to_string(objNumber)+".pcd";
  if (pcl::io::loadPCDFile<pcl::PointXYZRGB> (cad, *colouredPC3) == -1){
    PCL_ERROR ("Couldn't read file \n");
  }
  
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr graspPlaneSpheres(new pcl::PointCloud<pcl::PointXYZRGB>);        
  for(int i=0; i<this->graspPlaneCloud->points.size(); i++){
    float radius = 0.0015;
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene(new pcl::PointCloud<pcl::PointXYZRGB>);
    for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
      for(float phi=0; phi<3.14; phi = phi+5*0.017){
        ...
        pcl::PointXYZRGB punto;
        punto.x = radius * cos(theta) * sin(phi) + (this->graspPlaneCloud->points)[i].x;
        punto.y = radius * sin(theta) * sin(phi) + (this->graspPlaneCloud->points)[i].y;
        punto.z = radius * cos(phi) + (this->graspPlaneCloud->points)[i].z;
        punto.r = 0;
        punto.g = 255;
        punto.b = 0;
    ...
    (*graspPlaneSpheres) += (*scene); 
  }        
  
  *colouredPC3 += *graspPlaneSpheres;  
  
  cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_voxelGraspplane"+std::to_string(objNumber)+".pcd";
  cad2 = std::string("b")+"_voxelGraspplane"+std::to_string(objNumber)+".pcd";
  
  pcl::io::savePCDFileASCII (cad, *colouredPC3);
  std::cerr << "Saved " << (*colouredPC3).size () << " data points to " <<cad2<<"." << std::endl;             
                
  /*pcl::PointCloud<pcl::PointXYZRGB>::Ptr graspPlanePoints(new pcl::PointCloud<pcl::PointXYZRGB>);
  for(int num = 0; num < this->graspPlaneCloud->points.size(); num++){
    pcl::PointXYZRGB punto;
    punto.x = (this->graspPlaneCloud->points)[num].x;
    punto.y = (this->graspPlaneCloud->points)[num].y;
    punto.z = (this->graspPlaneCloud->points)[num].z;
    punto.r = 0;
    punto.g = 255;
    punto.b = 0;
    
    (graspPlanePoints->points).push_back(punto);
  }
  (*voxelCloud2) += (*graspPlanePoints);*/
  
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr colouredPC2(new pcl::PointCloud<pcl::PointXYZRGB>);
  cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_objVox"+std::to_string(objNumber)+".pcd";
  if (pcl::io::loadPCDFile<pcl::PointXYZRGB> (cad, *colouredPC2) == -1){
    PCL_ERROR ("Couldn't read file \n");
  }
  
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr initialPointsDraw(new pcl::PointCloud<pcl::PointXYZRGB>);
  radius = 0.0025;
  for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
    for(float phi=0; phi<3.14; phi = phi+5*0.017){
    
      pcl::PointXYZRGB punto;
      punto.x = radius * cos(theta) * sin(phi) + this->graspingPoints[0].getVector3fMap()[0];
      punto.y = radius * sin(theta) * sin(phi) + this->graspingPoints[0].getVector3fMap()[1];
      punto.z = radius * cos(phi) + this->graspingPoints[0].getVector3fMap()[2];
      punto.r = 255;
      punto.g = 0;
      punto.b = 0;
      
      (initialPointsDraw->points).push_back(punto);
    }
  } 
  ...
  for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
    for(float phi=0; phi<3.14; phi = phi+5*0.017){
      ...
      pcl::PointXYZRGB punto;
      punto.x = radius * cos(theta) * sin(phi) + this->graspingPoints[1].getVector3fMap()[0];
      punto.y = radius * sin(theta) * sin(phi) + this->graspingPoints[1].getVector3fMap()[1];
      punto.z = radius * cos(phi) + this->graspingPoints[1].getVector3fMap()[2];
      punto.r = 0;
      punto.g = 0;
      punto.b = 255;
  ...
  (*colouredPC2) += (*initialPointsDraw);
    
  /*cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_voxelGraspplaneInitialPoints"+std::to_string(objNumber)+".pcd";
  cad2 = std::string("b")+"_voxelGraspplaneInitialPoints"+std::to_string(objNumber)+".pcd";
...
//  this->chosenPointsNormalVoxel.push_back(this->chosenPointsNormal[1]);
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr candidatesInitialPointsDraw(new pcl::PointCloud<pcl::PointXYZRGB>);
  radius = 0.00125;
  for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
    for(float phi=0; phi<3.14; phi = phi+5*0.017){
      ...
      (candidatesInitialPointsDraw->points).push_back(punto);
    }
  } 
  ...
  (*colouredPC2) += (*candidatesInitialPointsDraw);
  
  
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr candidatesSpheres(new pcl::PointCloud<pcl::PointXYZRGB>);        
  for(int i=0; i<this->chosenPointsNormal[0]->size(); i++){
    float radius = 0.0015;
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene(new pcl::PointCloud<pcl::PointXYZRGB>);
    for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
      for(float phi=0; phi<3.14; phi = phi+5*0.017){
        ...
        pcl::PointXYZRGB punto;
        punto.x = radius * cos(theta) * sin(phi) + (*(this->chosenPointsNormal[0])).points[i].getVector3fMap()[0];
        punto.y = radius * sin(theta) * sin(phi) + (*(this->chosenPointsNormal[0])).points[i].getVector3fMap()[1];
        punto.z = radius * cos(phi) + (*(this->chosenPointsNormal[0])).points[i].getVector3fMap()[2];
        punto.r = 240;
        punto.g = 128;
        punto.b = 128;
    ...
    (*candidatesSpheres) += (*scene); 
  }        
  
  *colouredPC2 += *candidatesSpheres; 
  
  /*for(float contador=0; contador<this->chosenPointsNormal[0]->size(); contador++){
    
    pcl::PointXYZRGB punto;
    punto.x = (*(this->chosenPointsNormal[0])).points[contador].getVector3fMap()[0];
    punto.y = (*(this->chosenPointsNormal[0])).points[contador].getVector3fMap()[1];
    punto.z = (*(this->chosenPointsNormal[0])).points[contador].getVector3fMap()[2];
    punto.r = 255;
    punto.g = 0;
    punto.b = 0;
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr candidatesSpheres1(new pcl::PointCloud<pcl::PointXYZRGB>);        
  for(int i=0; i<this->chosenPointsNormal[1]->size(); i++){
    float radius = 0.0015;
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene(new pcl::PointCloud<pcl::PointXYZRGB>);
    for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
      for(float phi=0; phi<3.14; phi = phi+5*0.017){
        ...
        pcl::PointXYZRGB punto;
        punto.x = radius * cos(theta) * sin(phi) + (*(this->chosenPointsNormal[1])).points[i].getVector3fMap()[0];
        punto.y = radius * sin(theta) * sin(phi) + (*(this->chosenPointsNormal[1])).points[i].getVector3fMap()[1];
        punto.z = radius * cos(phi) + (*(this->chosenPointsNormal[1])).points[i].getVector3fMap()[2];
        punto.r = 135;
        punto.g = 206;
        punto.b = 250;
    ...
    (*candidatesSpheres1) += (*scene); 
  }        
  
  *colouredPC2 += *candidatesSpheres1; 
  
  /*for(float contador=0; contador<this->chosenPointsNormal[1]->size(); contador++){
    
    pcl::PointXYZRGB punto;
    punto.x = (*(this->chosenPointsNormal[1])).points[contador].getVector3fMap()[0];
    punto.y = (*(this->chosenPointsNormal[1])).points[contador].getVector3fMap()[1];
    punto.z = (*(this->chosenPointsNormal[1])).points[contador].getVector3fMap()[2];
    punto.r = 0;
    punto.g = 0;
    punto.b = 255;
  ...
  (*voxelCloud2) += (*candidatesInitialPointsDraw);*/
  
   
  cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_tillThumbCandidates"+std::to_string(objNumber)+".pcd";
  cad2 = std::string("b")+"_tillThumbCandidates"+std::to_string(objNumber)+".pcd";
  
  pcl::io::savePCDFileASCII (cad, *colouredPC2);
  std::cerr << "Saved " << (*colouredPC2).size () << " data points to " <<cad2<<"." << std::endl;
  /*******************************************************************************************************************/               
...
//  std::cout<<"VoxelRadius value="<<this->voxelRadius<<std::endl;
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr voxelCloud3(new pcl::PointCloud<pcl::PointXYZRGB>);
  voxelizeCloud<pcl::PointCloud<pcl::PointXYZRGB>::Ptr, 
                pcl::VoxelGrid<pcl::PointXYZRGB> >(colouredPC, this->voxelRadius, voxelCloud3);
  
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr newScene(new pcl::PointCloud<pcl::PointXYZRGB>);        
  for(int i=0; i<voxelCloud3->points.size(); i++){
    float radius = 0.0005;
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene(new pcl::PointCloud<pcl::PointXYZRGB>);
    for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
      for(float phi=0; phi<3.14; phi = phi+5*0.017){
        ...
        pcl::PointXYZRGB punto;
        punto.x = radius * cos(theta) * sin(phi) + (voxelCloud3->points[i]).x;
        punto.y = radius * sin(theta) * sin(phi) + (voxelCloud3->points[i]).y;
        punto.z = radius * cos(phi) + (voxelCloud3->points[i]).z;
        punto.r = 0;
        punto.g = 0;
        punto.b = 0;
  ...
  *voxelCloud3 += *candidatesSpheres1;   
  
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr candidatesSpheresFingers(new pcl::PointCloud<pcl::PointXYZRGB>);  
  for( int f=0;f<this->apertures.size();f++) {
    for(float contador=0; contador<this->candidatesNormalVoxel[f]->size(); contador++){      
      for(int i=0; i<this->chosenPointsNormal[1]->size(); i++){
        float radius = 0.0015;
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr scene(new pcl::PointCloud<pcl::PointXYZRGB>);
        ...
        for(float theta=0; theta<2*3.14; theta=theta+5*0.017){
          for(float phi=0; phi<3.14; phi = phi+5*0.017){
            ...
            pcl::PointXYZRGB punto;
            punto.x = radius * cos(theta) * sin(phi) + (*(this->candidatesNormalVoxel[f])).points[contador].getVector3fMap()[0];
            punto.y = radius * sin(theta) * sin(phi) + (*(this->candidatesNormalVoxel[f])).points[contador].getVector3fMap()[1];
            punto.z = radius * cos(phi) + (*(this->candidatesNormalVoxel[f])).points[contador].getVector3fMap()[2];
        ...
        (*candidatesSpheresFingers) += (*scene); 
      }
    }
  }        
  
  *voxelCloud3 += *candidatesSpheresFingers; 
  ...
  /*for( int f=0;f<this->apertures.size();f++) {
     for(float contador=0; contador<this->candidatesNormalVoxel[f]->size(); contador++){
      ...
      pcl::PointXYZRGB punto;
      punto.x = (*(this->candidatesNormalVoxel[f])).points[contador].getVector3fMap()[0];
      punto.y = (*(this->candidatesNormalVoxel[f])).points[contador].getVector3fMap()[1];
      punto.z = (*(this->candidatesNormalVoxel[f])).points[contador].getVector3fMap()[2];
  ...
  (*voxelCloud3) += (*candidatesInitialPointsDraw);*/
   
  std::string cad = std::string("/ignacio/Desktop/iros_2023/catkin_ws/src/GeoGraspEvo/experiments/b")+"_FinalCandidates"+std::to_string(objNumber)+"_"+std::to_string(side)+".pcd";
  std::string cad2 = std::string("b")+"_FinalCandidates"+std::to_string(objNumber)+"_"+std::to_string(side)+".pcd";
  
  pcl::io::savePCDFileASCII (cad, *voxelCloud3);
  std::cerr << "Saved " << (*voxelCloud3).size () << " data points to " <<cad2<<"." << std::endl;
  /*******************************************************************************************************************/               
