...
void GeoGraspEvo::setApertures(const std::vector<std::vector<int>> & open) {
  ...
  for(int i=0;i<apertures.size();i++){
    for(int j=0;j<apertures[i].size();j++){
      std::cout<<i<<" "<<j<<" "<<apertures[i][j]<<std::endl;
    }
  }*/
...
void GeoGraspEvo::compute() {
  ...
  voxelizeCloud<pcl::PointCloud<pcl::PointXYZ>::Ptr, 
                pcl::VoxelGrid<pcl::PointXYZ> >(originalCloud, voxelRadius, voxelCloud);
  ...
  getClosestPointsByRadius(this->graspingPoints[0], graspRadius, cloud, 
  ...
  getClosestPointsByRadius(this->graspingPoints[1], graspRadius, cloud, 
                           this->objectNormalCloud, candidatePoints, candidatePointsNormal);
...
void GeoGraspEvo::getBestGraspingPoints(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,    
  ...
  for (size_t i = 0; i < this->chosenPointsNormalVoxel[0]->points.size(); i++) {
    ...
    for (size_t j = 0; j < this->chosenPointsNormalVoxel[1]->size(); j++) {
      for (size_t k = 0; k < this->candidatesNormal[0][j]->size(); k++) {
        ...
        for (size_t l = 0; l < this->candidatesNormal[1][j]->size(); l++) {
